            ...
            while 1:
                try:
                    ...
                    if msg:
                        ...
                        del_obj = re.fullmatch(''.join(reproto.DELETE.value), msg)
                        ...
                        elif del_obj is not None:
                            try:
                                remove(f'io_server_files/{del_obj[1]}')
                                file_contents = bytes("File Deleted", 'utf-8')
                                connfd.sendall(file_contents)
                            except IOError:
                                connfd.sendall(bytes("404: file not found", 'utf-8'))
                            del_obj = None
